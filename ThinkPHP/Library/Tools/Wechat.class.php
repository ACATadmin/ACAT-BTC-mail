...
class Wechat {
	 ...
	 * @return string	 
	 */
	public function getOAuthRedirect($callback=null, $state='', $scope='snsapi_base'){
		if($callback==null){
			//$callback = 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING'];
			if(substr($_SERVER['QUERY_STRING'], 2, 5)=='obile') {
				$callback = 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['QUERY_STRING'];
			}else{
				$callback = 'http://'.$_SERVER['HTTP_HOST'].'?'.$_SERVER['QUERY_STRING'];
			}
			
			$callback = str_replace('.html','/',$callback);
			//vde(urlencode($callback));
		}
		//vde( $_SERVER );echo $callback;exit;
		
		$callback = $callback."&";
		
		// file_put_contents("D:\\wamp\\www\\the1stVotingBloc\\aa.txt", $_SERVER['QUERY_STRING'] ."++++". $callback."======",FILE_APPEND );
	...
	public function getOauthUserInfoTest($access_token, $openid) {
		$params = array(
				'access_token'  => $access_token,
				'openid'        => $openid,
				'lang'          => 'zh_CN'
	...
	public function getQRUrlOne($scene_id = '', $limit = true, $expire = 1800) {
		$this->qrcode($scene_id, $limit, $expire);
		return self::QRCODE_SHOW_URL.'?ticket=' . $this->ticket;
	}
	 ...
	 * @return json|boolean json 直接可赋给JSAPI接口使用，boolean错误
	 */
	public function unifiedOrders($res) {
		$order_info=$res['pay_order_info'];
		$body=$order_info['body'];
		$appid="";
		$mch_id="";
		if( isset($order_info['source_device']) && $order_info['source_device']=='android' ){
			$appid="";
			$mch_id="";
		}
		/*
		$appid="wxea6d17fe28b94a59";
		$mch_id="1338926401";
		*/
		//$mch_id=$order_info['source_device'];
		...
		$params = array(
			...
			'mch_id'           => $mch_id,
			//'device_info'		=> 1000,
			'nonce_str'        => strtolower(self::_getRandomStr()),
			'notify_url'       => $notify_url,
			'out_trade_no'     => $orderId,
			'total_fee'        => $money * 100, // 转换成分
		...
		// 生成签名
		$params['sign'] = self::_getOrderMd5s($params);
		//ksort($params);
		//return $params;
		
		
		//$paramss = self::_getOrdersMd5($params);
		$data = self::_array2Xml($params);
		$data = $this->http(self::UNIFIED_ORDER_URL, $data, 'POST');
		...
		// $data=array('data'=>$data,'datas'=>$params);
		$data['nonce_str'] = strtolower($data['nonce_str']);
		if ($data['return_code'] == 'SUCCESS') {
			/*
		    $data['nonce_strs']=strtolower($params['nonce_str']);
		    $data['old_sign']=$params['sign'];
		
		    $data['spbill_create_ipS']=$params;


		    //$data['pay_signs']=$this->pay_signs($params); 
			...
			*/
			...
			$retData = array(
					//"sss"=>NOW_TIME,
					"appid"=>$data['appid'],
					"noncestr"=>$params['nonce_str'],//"4768504665cb7c1141fe009028b56565",
					"package"=>"Sign=WXPay",
					"partnerid"=>$data['mch_id'],
					"prepayid"=>$data['prepay_id'],	//"wx201604131130569575cfeecd0917585241",
					"timestamp"=>NOW_TIME,			//,
					//"source_device"=>$order_info['source_device'],
					//"sign"=>"26C207ACD31007F7183D025893B552D2"
			);
			$retData['sign'] = self::_getOrderMd5s($retData);
	 ...
	 * 本地MD5签名
	 */
	private function _getOrderMd5s($params) {
		ksort($params);
		$params['key'] = "SHANGHAISHENWOXINXIKEJIYOUXIANGO";
                //vde($params);
	...
	private function pay_signs($params){


		$stringA="appid=".$params['appid']."&body=".$params['body']."&device_info=1000&mch_id=".$params['mch_id']."&nonce_str=".$params['nonce_str']; 


		$stringSignTemp=$stringA."&key=yhpc2016412hyhhyhdguidcvng4785px"; 
	 ...
	 * 本地MD5签名
	 */
	private function _getOrdersMd5($params) {
		ksort($params);
		$params['key'] = "yhpc2016412hyhhyhdguidcvng4785px";
	
		$stringSignTemp=http_build_query($params);
