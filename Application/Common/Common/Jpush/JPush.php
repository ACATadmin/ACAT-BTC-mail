...
class JPush {
    ...
    public function __construct($appKey, $masterSecret, $logFile=self::DEFAULT_LOG_FILE, $retryTimes=self::DEFAULT_MAX_RETRY_TIMES) {
        if (is_null($appKey) || is_null($masterSecret)) {
            ...
            $j_arr['error']="appKey and masterSecret must be set.";
        ...
        if (!is_string($appKey) || !is_string($masterSecret)) {
            ...
            $j_arr['error']="Invalid appKey or masterSecret";
            $j_arr['jpush_time']=date('Y-m-d H:i:s',NOW_TIME);
            D('JpushLog')->add($j_arr);
        }
        $this->appKey = $appKey;
     ...
     */
    public function _request($url, $method, $body=null, $times=1,$regid=null) {
        ...
        if ($errorCode) {
            if ($errorCode === 28) {
                ...
                $j_arr['error']="Response timeout. Your request has probably be received by JPush Server,please check that whether need to be pushed again.";
                $j_arr['jpush_time']=date('Y-m-d H:i:s',NOW_TIME);
                D('JpushLog')->add($j_arr);
                ...
                $j_arr['jpush_id']=  serialize($regid);
                $j_arr['error']="Response timeout, maybe cause by old CURL version. Your request has probably be received by JPush Server, please check that whether need to be pushed again.";
                $j_arr['jpush_time']=date('Y-m-d H:i:s',NOW_TIME);
                D('JpushLog')->add($j_arr);
                ...
                $j_arr['jpush_id']=  serialize($regid);
                $j_arr['error']="Connect timeout. Please retry later. Error:" . $errorCode . " " . curl_error($ch);
                $j_arr['jpush_time']=date('Y-m-d H:i:s',NOW_TIME);
                D('JpushLog')->add($j_arr);
            } else {
