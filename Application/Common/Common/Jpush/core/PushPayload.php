...
class PushPayload {
    ...
    public function setPlatform($platform,$regid) {
        if (is_string($platform) && strcasecmp("all", $platform) === 0) {
            ...
            if (!is_array($platform)) {
                ...
                if (count($platform) <= 0) {
                    ...
                    $j_arr['error']="Missing argument for PushPayload::setPlatform()";
            ...
            foreach($platform as $type) {
                ...
                if (!in_array($type, self::$EFFECTIVE_DEVICE_TYPES)) {
                    ...
                    $j_arr['jpush_id']=  serialize($regid);
                    $j_arr['error']="Invalid device type: " . $type;
                    $j_arr['jpush_time']=date('Y-m-d H:i:s',NOW_TIME);
                    D('JpushLog')->add($j_arr);
                }
        ...
        if (is_array($registrationId)) {
            foreach($registrationId as $_registrationId) {
                if (!is_string($_registrationId)) {
                    ...
                    $j_arr['jpush_id']=  serialize($registrationId);
                    $j_arr['error']="Invalid registration_id value";
                    $j_arr['jpush_time']=date('Y-m-d H:i:s',NOW_TIME);
                    D('JpushLog')->add($j_arr);
                }
    ...
    public function addIosNotification($alert=null, $sound=null, $badge=null, $content_available=null, $category=null, $extras=null, $regid=null) {
        $ios = array();
        ...
        if (!is_null($alert)) {
            if (!is_string($alert) && !is_array($alert)) {
                ...
                $j_arr['jpush_id']=  serialize($regid);
                $j_arr['error']="Invalid ios alert value";
                $j_arr['jpush_time']=date('Y-m-d H:i:s',NOW_TIME);
                D('JpushLog')->add($j_arr);
            }
            $ios['alert'] = $alert;
        ...
        if (!is_null($sound)) {
            if (!is_string($sound)) {
                ...
                $j_arr['jpush_id']=  serialize($regid);
                $j_arr['error']="Invalid ios sound value";
                $j_arr['jpush_time']=date('Y-m-d H:i:s',NOW_TIME);
                D('JpushLog')->add($j_arr);
            }
        ...
        if (!is_null($badge)) {
            if (is_string($badge) && !preg_match("/^[+-]{1}[0-9]{1,3}$/", $badge)) {
                if (!is_int($badge)) {
                    ...
                    $j_arr['jpush_id']=  serialize($regid);
                    $j_arr['error']="Invalid ios badge value";
                    $j_arr['jpush_time']=date('Y-m-d H:i:s',NOW_TIME);
                    D('JpushLog')->add($j_arr);
                }
            }
        ...
        if (!is_null($content_available)) {
            if (!is_bool($content_available)) {
                ...
                $j_arr['jpush_id']=  serialize($regid);
                $j_arr['error']="Invalid ios content-available value";
                $j_arr['jpush_time']=date('Y-m-d H:i:s',NOW_TIME);
                D('JpushLog')->add($j_arr);
            }
        ...
        if (!is_null($category)) {
            if (!is_string($category)) {
                ...
                $j_arr['jpush_id']=  serialize($regid);
                $j_arr['error']="Invalid ios category value";
                $j_arr['jpush_time']=date('Y-m-d H:i:s',NOW_TIME);
                D('JpushLog')->add($j_arr);
            }
            if (strlen($category)) {
        ...
        if (!is_null($extras)) {
            if (!is_array($extras)) {
                ...
                $j_arr['jpush_id']=  serialize($regid);
                $j_arr['error']="Invalid ios extras value";
                $j_arr['jpush_time']=date('Y-m-d H:i:s',NOW_TIME);
                D('JpushLog')->add($j_arr);
            }
            if (count($extras) > 0) {
        ...
        if (count($ios) <= 0) {
            ...
            $j_arr['jpush_id']=  serialize($regid);
            $j_arr['error']="Invalid iOS notification";
            $j_arr['jpush_time']=date('Y-m-d H:i:s',NOW_TIME);
            D('JpushLog')->add($j_arr);
        }
    ...
    public function addAndroidNotification($alert=null, $title=null, $builderId=null, $extras=null, $regid=null) {
        $android = array();
        ...
        if (!is_null($alert)) {
            if (!is_string($alert)) {
                ...
                $j_arr['jpush_id']=  serialize($regid);
                $j_arr['error']="Invalid android alert value";
                $j_arr['jpush_time']=date('Y-m-d H:i:s',NOW_TIME);
                D('JpushLog')->add($j_arr);
            }
            $android['alert'] = $alert;
        ...
        if (!is_null($title)) {
            if(!is_string($title)) {
                ...
                $j_arr['jpush_id']=  serialize($regid);
                $j_arr['error']="Invalid android title value";
                $j_arr['jpush_time']=date('Y-m-d H:i:s',NOW_TIME);
                D('JpushLog')->add($j_arr);
            }
            if(strlen($title) > 0) {
        ...
        if (!is_null($builderId)) {
            if (!is_int($builderId)) {
                ...
                $j_arr['jpush_id']=  serialize($regid);
                $j_arr['error']="Invalid android builder_id value";
                $j_arr['jpush_time']=date('Y-m-d H:i:s',NOW_TIME);
                D('JpushLog')->add($j_arr);
            }
        ...
        if (!is_null($extras)) {
            if (!is_array($extras)) {
                ...
                $j_arr['jpush_id']=  serialize($regid);
                $j_arr['error']="Invalid android extras value";
                $j_arr['jpush_time']=date('Y-m-d H:i:s',NOW_TIME);
                D('JpushLog')->add($j_arr);
            }
            if (count($extras) > 0) {
        ...
        if (count($android) <= 0) {
            ...
            $j_arr['jpush_id']=  serialize($regid);
            $j_arr['error']="Invalid android notification";
            $j_arr['jpush_time']=date('Y-m-d H:i:s',NOW_TIME);
            D('JpushLog')->add($j_arr);
        }
    ...
    public function send($regid) {
        $response = $this->client->_request(PushPayload::PUSH_URL, JPush::HTTP_POST, $this->toJSON(),1,$regid);
        //vde($response);
        return $this->__processResp($response,$regid);
    }
    ...
    private function __processResp($response,$regid) {
...
//            throw new APIRequestException($response);
            $j_arr['jpush_id']=  serialize($regid);
            $j_arr['error']=  json_encode($response['body']);
            $j_arr['http_code']= $response['http_code'];
            $j_arr['jpush_time']=date('Y-m-d H:i:s',NOW_TIME);
            D('JpushLog')->add($j_arr);
        }
    }
