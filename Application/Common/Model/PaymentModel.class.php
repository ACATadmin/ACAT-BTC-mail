...
class PaymentModel extends BaseModel{
	...
	//获取支付详细信息
	/*
	public function getInfo( $pack_id ){
		
		$info = $this->where( array('pack_id'=>$pack_id) )->find();
		
		return $info;
	}
	
	public function getList(){
		$list = $this->where()->order('pay_order desc')->select();
	...
	*/
	...
	public function format( $info ){
		...
		//如果是支付宝，获取支付宝下对应的网银
		if( $info['pay_code'] == 'alipay' ){
			$info['bank'] = array();
			
			$pay_config = unserialize( $info['pay_config'] );
			unset( $pay_config['alipay_account'] );
			unset( $pay_config['alipay_partner'] );
			unset( $pay_config['alipay_key'] );			

			$pay_config['bankimg'] = C('PAYMENT_PATH') . '/lib/alipay/bankimg';
			$info['bank'] = $pay_config;
	...
	//===================================================================================
	//支付处理
	public function do_pay( $oInfo,$payCode='' ){
		...
		//查询出订单付款方式
		if( empty($payCode) && $oInfo['pay_code'] ){
			$payCode = $oInfo['pay_code'];
		}
		
		
		//$oInfo['body'] = '测试订单';
		
		//echo $payCode;vde( $oInfo );
		
		$payModel 	= D( ucfirst($payCode), 'Payment');//D($payCode, 'Payment');
		//vde(ucfirst($payCode));
		//获取付款码（执行付款，或返回付款状态）
		if( isMobile() && $oInfo['bank']=='' && method_exists($payModel,'getPayCode_Mobile') ){	//手机端访问
	...
	public function getList( $map ){
		$list = parent::getList($map);
		
		$list = getIdIndexArr($list,'pay_code');
