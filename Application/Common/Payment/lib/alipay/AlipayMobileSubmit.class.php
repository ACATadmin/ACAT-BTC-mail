...
class AlipaySubmit {
	...
	function buildRequestMysign_mobile($para_sort) {
		//把数组所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串
		$prestr = createLinkstring_orig($para_sort);
	...
	function buildRequestPara($para_temp) {
		...
		//签名结果与签名方式加入请求提交参数组中
		$para_sort['sign_nourlencode'] = $mysign;
	...
	function buildRequestPara_mobile($para_temp) {
		//除去待签名参数数组中的空值和签名参数
		$para_filter = paraFilter($para_temp);
		...
		//生成签名结果
		$mysign = $this->buildRequestMysign_mobile($para_sort);
	...
	function createSortPara($para_temp){
		//除去待签名参数数组中的空值和签名参数
		$para_filter = paraFilter($para_temp);
	...
	function buildRequestForm($para_temp, $method, $button_name) {
		//待请求参数数组
		$para = $this->buildRequestPara_mobile($para_temp);
		$para['sign'] = $para['sign_nourlencode'];
		unset($para['sign_nourlencode']);
