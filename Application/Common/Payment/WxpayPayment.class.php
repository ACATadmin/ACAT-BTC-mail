...
class WxpayPayment extends BasePayment{
	...
	public function getOrderSnValue(){
		...
		$order_sn = $retArr[$this->order_sn_name];
		
		$order_sn = str_replace("mob", "", $order_sn);
	...
	public function getPayCode( $order ){
		if( isMobile() ){	//手机处理
			...
			return $this->getPayCode_Pc( $order );
		}
	}
	...
	public function getPayCode_Mobile( $order ){
		...
		//echo "<br/><br/><br/><br/><br/><br/>ppp";vde( $order );
		...
		$order['subject'] = ($order['subject'])?$order['subject']:$order['body'];
// 		echo $order['subject'];exit;
		$unifiedOrder->setParameter("body",$order['subject']);					//商品描述
		//自定义订单号，此处仅作举例
		$order['order_sn'] = 'mob'.$order['order_sn'];
		...
		//模版输出
		ob_start();
		$successJumpUrl = $order['successJumpUrl'];
		if( empty($successJumpUrl) ){
			$successJumpUrl = U('Mobile/Account/order');
		}
	...
	//PC端支付
	public function getPayCode_Pc( $order ){
		//vde($order);
