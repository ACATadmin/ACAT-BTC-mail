...
/* 获取父分类名*/
function getgcnameByid($gcid){
    if(!$gcid){
        return false;
    }
    $threegcinfo=M('GoodsCategory')->field('parent_id,cate_name')->find($gcid);
    $twogcinfo=M('GoodsCategory')->where(array('id'=>$threegcinfo['parent_id']))->field('parent_id,cate_name')->find();
    $onegcinfo=M('GoodsCategory')->where(array('id'=>$twogcinfo['parent_id']))->field('parent_id,cate_name')->find();
    if($threegcinfo && $twogcinfo && $onegcinfo){
        return $onegcinfo['cate_name'].'>>'.$twogcinfo['cate_name'].'>>'.$threegcinfo['cate_name'];
    }
    return ;
}
/* 解析插件数据列表定义规则*/
...
function int_to_rolestring(&$data,$map=array('realname_status'=>array(1=>'待审核',2=>'审核通过',3=>'审核不通过',0=>'未认证 '))) {
    if($data === false || $data === null ){//实名认证 0：未认证 1：待审核 2：审核通过 （已实名）3：审核不通过
        return $data;
    }
    $data = (array)$data;
    foreach ($data as $key => $row){
...
function int_to_string2(&$data) {
    ...
    $data = (array)$data;
    foreach ($data as $key => $row){
//        switch($row['order_status']){//订单状态。0，未付款；1，未发货；2，已发货；3，已完成；
//            case 0:
...
//        }
        if($row['pay_status']==0){
            ...
            $data[$key]['status_txt'] = '待付尾款';
        }else if($row['pay_status']==2 && $row['shipping_status']==0){
            $data[$key]['status_txt'] = '待发货';
        }else if($row['pay_status']==2 && $row['shipping_status']==1){
            $data[$key]['status_txt'] = '待收货';
        }else if($row['pay_status']==2 && $row['shipping_status']==2){
            $data[$key]['status_txt'] = '已收货';
        }
        switch($row['pay_code']){//订单状态。0，未付款；1，未发货；2，已发货；3，已完成；
            case 'alipay':
                $data[$key]['pay_way_txt'] = '支付宝支付';
                break;
            case 'wxpay':
                $data[$key]['pay_way_txt'] = '微信支付';
                break;
            case 'balance':
                $data[$key]['pay_way_txt']='余额支付';
                break;
        }
...
function int_to_onlinestring(&$data,$map=array('online'=>array(1=>'已上线',0=>'已下线'))) {
    if($data === false || $data === null ){
        return $data;
    }
    $data = (array)$data;
