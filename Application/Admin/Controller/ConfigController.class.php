    ...
    public function save($config){
        if($config && is_array($config)){
            ...
            foreach ($config as $name => $value) {
                //判断
                if($name=='NOTICE_SAVE_TIME'){
                    $value=trim($value);
                    if($value<=0 || !is_numeric($value) || strstr($value,'.')){
                        $this->error('预告保留时间必须是正整数');
                    }
                }
                if($name=='PLATFORM_GET_MONEY'){
                    $value=trim($value);
                    if($value<0 || $value>100 || !is_numeric($value) || strstr($value,'.')){
                        $this->error('平台分佣比例必须是0到100之间的整数');
                    }
                }
//                if($name=='PLATFORM_MOBILE'){
//                    $value=trim($value);
//                    if(!preg_match('/^0?(13[0-9]|15[0-9]|17[0-9]|18[0-9]|14[57])[0-9]{8}$/',$value)){
//                        $this->error('手机格式不正确'); 
//                    }
//                }
